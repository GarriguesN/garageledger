...
console.log("Benchmarking maintenance_tasks loop (Optimized)...");
...
const updateMT = db.prepare(`
    UPDATE maintenance_tasks
    SET preset_key =
        CASE part_name
        ${presets.map(([partName, presetKey]) => `WHEN '${partName.replace(/'/g, "''")}' THEN '${presetKey}'`).join('\n        ')}
        END
    WHERE preset_key IS NULL OR preset_key=''
      AND part_name IN (${presets.map(([partName]) => `'${partName.replace(/'/g, "''")}'`).join(', ')})
`);
updateMT.run();
...
console.log("Benchmarking expenses loop (Optimized)...");
...
const updateExp = db.prepare(`
    ...
    SET tipo_id =
        CASE tipo
        ${Object.entries(tipoIdMap).map(([label, id]) => `WHEN '${label.replace(/'/g, "''")}' THEN '${id}'`).join('\n        ')}
        END
    WHERE (tipo_id IS NULL OR tipo_id='')
      AND tipo IN (${Object.keys(tipoIdMap).map(label => `'${label.replace(/'/g, "''")}'`).join(', ')})
`);
updateExp.run();
